...
export const renderVersionWarning = (npmVersion: string) => {
  ...
  if (currentVersion.includes('beta')) {
    logger.warn('  You are using a beta version of smb-nest-cli.');
    logger.warn('  Please report any bugs you encounter.');
  } else if (currentVersion.includes('next')) {
    logger.warn(
      '  You are running smb-nest-cli with the @next tag which is no longer maintained.',
    );
    logger.warn('  Please run the CLI with @latest instead.');
  } else if (currentVersion !== npmVersion) {
    logger.warn('  You are using an outdated version of smb-nest-cli.');
    logger.warn(
      '  Your version:',
      currentVersion + '.',
...
function checkForLatestVersion(): Promise<string> {
  return new Promise((resolve, reject) => {
    https
      .get(
        'https://registry.npmjs.org/-/package/smb-nest-cli/dist-tags',
        (res) => {
...
export const getNpmVersion = () =>
  ...
  checkForLatestVersion().catch(() => {
    try {
      return execSync('npm view smb-nest-cli version').toString().trim();
    } catch {
      return null;
    }
  });

...
export default function (
  ...
  if (allMethods.length > 0) {
    schema.pre(allMethods as any, async function <
      T extends Deleted & DeletedMethods,
  ...
  if (hasAggregateInOption(methods)) {
    schema.pre('aggregate', async function (this: Aggregate<unknown>) {
      if (onlyDeletedInOptions(this)) {
        ...
        deletedIsNotAlreadyInAggregation(this) &&
        notWithDeletedInOptions(this)
      ) {
...
function deletedIsNotAlreadyInAggregation(
  aggregation: Aggregate<unknown>,
): boolean {
  const matches = aggregation.pipeline().filter(isPipelineMatch);
  return !matches.some((match: PipelineStage.Match) =>
    Object.keys(match['$match']).includes('deleted'),
  );
}

function isPipelineMatch(
  pipeline: PipelineStage,
): pipeline is PipelineStage.Match {
